...
# Load a pre-trained Word2Vec model (use your desired pre-trained model)
hugvector_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
model_path = os.path.join(hugvector_root, "models", "GoogleNews-vectors-negative300.bin")

model = KeyedVectors.load_word2vec_format (model_path, binary=True, limit=50000)

# Make sure you use your own data and code
...
# Init and builds the vector db
hug_db = HugDB()
hug_db.build_index(vectors)


# Performs a search using a query vector
